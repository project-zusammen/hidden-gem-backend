...
class TestUserEndpoints(TestCase):
    ...
    @patch("app.main.controller.user_controller.updated_user")
    ...
    @patch("app.main.controller.user_controller.updated_user_status")
    def test_update_user_status(self, mock_update_user):
        ...
        # ACT
        with self.app.test_client() as client:
            response = client.put(f"/api/user/{public_id}/status", json=user_data)
            res = response.get_json()["data"]
